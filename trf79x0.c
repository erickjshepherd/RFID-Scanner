...
// IRQ Handler
//
// NOTE: currently TimerSet, Timer0Dis, and TimerInteruptHandler must be
// implemented by the user.
//
//*****************************************************************************
extern void TimerSet(uint16_t timeout_ms, uint8_t * timeout_flag);
extern void Timer0Dis(void);
tTRF79x0IRQFlag
TRF79x0IRQHandler(uint16_t ui16TimeOut)
{
    ...
    if (g_time_out_flag == 0x01) {
            ...
            if (pui8IRQBuffer[0] & IRQ_STATUS_RX_COMPLETE) {
                        ...
                        (g_fifo_bytes_received < NFC_FIFO_SIZE))
                {
                    ...
                    ui8FifoStatusLength &= 0x7F;
								...
								int x;
								for(x = 0; x < g_fifo_bytes_received; x++){
									g_sRXState.pucBuffer[x] = g_fifo_buffer[x];
								}
								
								g_sRXState.puiLength[0] = g_fifo_bytes_received;
								
                TRF79x0ResetFifoCommand(); // remove
        ...
        else if (pui8IRQBuffer[0] == IRQ_STATUS_RX_COMPLETE) // changed from ==
        {
						/*
            //
            // Read the FIFO status and FIFO into g_nfc_buffer
            ...
            // Handle the case for P2P Initiator Mode where IRQ is triggered  our board
            // with value 0xC0 - TODO
            if(pui8IRQBuffer[0] & IRQ_STATUS_TX_COMPLETE)
            {
								ui8FifoStatusLength = TRF79x0ReadRegister(TRF79X0_FIFO_STATUS_REG); // added
								
								TRF79x0ReadRegisterContinuous(TRF79X0_FIFO_REG, g_fifo_buffer, ui8FifoStatusLength); // added
