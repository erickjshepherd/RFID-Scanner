...
{
    //TRF79x0Command(TRF79X0_STOP_DECODERS_CMD); // Error Line
            	  ...
            	  unsigned char ucNbSlots, unsigned char *pucMask, unsigned char ucMaskLen)
{
    ...
    //
	  uint32_t test = 100;
    TRF79x0Transceive(g_pucCmd, 3, 0, pucResponse, &test, 0, TRF79X0_TRANSCEIVE_CRC); // Error Line
	...
	while((uiRxSize != 10) && (slot < 16))
	{
	    ...
	    //
		ISO15693NextSlot();
		TRF79x0ReceiveAgain(pucResponse, &test); // Error Line
		slot++;
	};
...
ISO15693StayQuiet(unsigned char  *pucUID)
{
    ...
    //
    //TRF79x0Transceive(g_pucCmd, 10, 0, 0, 0, 0, TRF79X0_TRANSCEIVE_TX_CRC); // Error Line
	return 0;
}
