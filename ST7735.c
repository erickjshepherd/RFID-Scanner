...
// RESET (pin 3) connected to PD7 (GPIO)
...
void static commonInit(const uint8_t *cmdList) {
  ...
  GPIO_PORTA_DIR_R |= 0x48;             // make PA3,6 out
	GPIO_PORTD_DIR_R |= 0x80;							// make PD7 out
  GPIO_PORTA_AFSEL_R &= ~0x48;          // disable alt funct on PA3,6
	GPIO_PORTD_AFSEL_R &= ~0x80;          // disable alt funct on PD7
  GPIO_PORTA_DEN_R |= 0x48;             // enable digital I/O on PA3,6
	GPIO_PORTD_DEN_R |= 0x80;             // enable digital I/O on PD7
                                        // configure PA3,6 as GPIO
  GPIO_PORTA_PCTL_R = (GPIO_PORTA_PCTL_R&0xF0FF0FFF)+0x00000000;
	GPIO_PORTD_PCTL_R = (GPIO_PORTD_PCTL_R&0x0FFFFFFF)+0x00000000;
  GPIO_PORTA_AMSEL_R &= ~0x48;          // disable analog functionality on PA3,6
